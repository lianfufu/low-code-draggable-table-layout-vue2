...
<template>
  <div class="wrap" :style="{paddingBottom:padding+'px',paddingTop:padding+'px'}" @click="sourceTargetClickIsTD">
    ...
    <cell-operation-bar v-show="isClickedAtOperationBar||(isShowOperationBar&&writableIsClickTD)"
                        ...
                        :isClickedAtOperationBar.sync="isClickedAtOperationBar" :positionVal="operationBarPosition"/>
    <table ref="mytable" class="table" :border="isShowBorder?1:0">
      ...
      <tr v-for="(item,index) in tableDataArr2" :style="{height:rowHeights[index]+'px'}" :key="index">
        <td :data-rowIndex="index" :data-colIndex="index2" :data-colSpan="item2[0]?item2[0].colSpan:1"
            :data-rowSpan="item2[0]?item2[0].rowSpan:1" v-if="item2.length>0" :colspan="item2[0]?item2[0].colSpan:1"
            :rowspan="item2[0]?item2[0].rowSpan:1"
            :class="[isShowBorder?'':'no-border',isSelectedCell(index,index2)?'selected-cell':'']"
            v-for="(item2,index2) in item" :key="index2" @mousedown="tdMouseDown" @mousemove="tdMouseMove"
            ...
            class="resizable-cell flex-td" :style="getCellStyle(columnWidths,item2,index2)">
...
export default {
  ...
  mounted() {
    this.$store.commit("setCurComponent",{six:666});
  ...
  computed: {
    ...
    isClickedTD() {
      ...
      if (!this.curComponent.component || this.curComponent.component === "McTable" || this.curComponent.component === "MCTextContainer") {
        return true;//因为设定过当点击td，这个时候没有设置component属性
      }
      console.log("其他情况才给的false", this.curComponent, this.curComponent.component === "McTable");
      return false;
    },
  },
  watch: {
  ...
  methods: {
    ...
    showTableConfig(item, rowIndex, colIndex, rowSpan, colSpan) {
      ...
      this.$store.commit("setCurComponent",item[0].cellFieldsVal);
      this.$store.commit("setCurFields",item[0].cellFields);//字段定义
    },
